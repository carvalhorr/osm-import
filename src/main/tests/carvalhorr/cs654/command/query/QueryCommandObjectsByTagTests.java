package carvalhorr.cs654.command.query;

import java.io.FileNotFoundException;
import java.util.List;

import org.junit.Test;

import carvalhorr.cs654.exception.FailedToCompleteQueryException;
import carvalhorr.cs654.persistence.data.TestDataProvider;

public class QueryCommandObjectsByTagTests extends BaseQueryCommandTests {

	private String defaultFileName = "tag-" + TestDataProvider.tagKeyForQuery + "-" + TestDataProvider.tagValueForQuery;
	private String defaultOutputFormat = "csv";

	private static final String QUERY_TYPE = "objects-by-tag";

	@Test
	public void queryEditingSummaryWithNoFormatProvidedShouldCreateCsvFile()
			throws FileNotFoundException, FailedToCompleteQueryException {
		List<String> args = createArgs(QUERY_TYPE, "a-file-name.csv", dataProvider.SCHEMA_NAME, "");

		args.add("--tag-name");
		args.add(TestDataProvider.tagKeyForQuery);

		args.add("--tag-value");
		args.add(TestDataProvider.tagValueForQuery);

		executeTest(args, defaultFileName, "a-file-name.csv", "", TestDataProvider.OBJECTS_BY_TAG_CSV);
	}

	@Override
	protected String getExpectedOutput(String file, String format) {
		return "PROCESSING FILE\n" + "Area name : test_data_query\n" + "Query type : " + QUERY_TYPE + "\n"
				+ "Output file name : " + ("".equals(file) ? "default file name" : file) + "\n" + "Output format : "
				+ ("".equals(format) ? defaultOutputFormat : format) + "\n"
				+ "Database properties file : database.properties\n" + "Tag name:tag2\n" + "Tag value:value2\n" + "\n"
				+ "Query finished.\n" + "File saved in: " + super.fullFilename;
	}

}
